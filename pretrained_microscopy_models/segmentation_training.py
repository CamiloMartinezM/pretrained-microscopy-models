...
from . import io, losses, util
from . import metrics as pmm_metrics
...
def create_segmentation_model(architecture, encoder, encoder_weights, classes, activation=None):
    ...
    assert classes != 2, (
        "If you are doing binary classification then set \
    ...
    if initial_weights == "imagenet" and encoder in ["dpn68b", "dpn92", "dpn137", "dpn107"]:
    ...
    try:
        ...
        raise ValueError(
            "%s does not support dilated mode needed for %s." % (encoder, architecture)
        )
...
def segmentation_models_inference(
    io,
    model,
    preprocessing_fn,
    device=None,
    ...
    io = np.pad(
        io,
        [(stride_size // 2, stride_size // 2), (stride_size // 2, stride_size // 2), (0, 0)],
        mode="reflect",
    )
    ...
    for batch_arr in divide_batch(arr_out, batch_size):
        arr_out_gpu = torch.from_numpy(batch_arr.transpose(0, 3, 1, 2).astype("float32")).to(
            device
        )
    ...
    output = output[
        :, :, stride_size // 2 : -stride_size // 2, stride_size // 2 : -stride_size // 2, :
    ]
...
def train_segmentation_model(
    ...
    assert len(class_values) != 2, (
        "Two classes is binary classification.  Just specify the posative class value."
    )
    assert patience is not None or epochs is not None, (
        "Need to set patience or epochs to define a stopping point."
    )
    ...
    metrics = [
        ...
        pmm_metrics.IoU(device=device),
        pmm_metrics.PerClassIou(
            JaccardIndex(task="multiclass", num_classes=2, average=None).to(device),
            class_index=0,
            class_name="grain_iou",
        ),
        pmm_metrics.PerClassIou(
            JaccardIndex(task="multiclass", num_classes=2, average=None).to(device),
            class_index=1,
    ...
    while True:
        ...
        state["train_iou"].append(train_logs["weighted_iou"])
        state["valid_iou"].append(valid_logs["weighted_iou"])

        # save the model
        torch.save(state, os.path.join(save_folder, "checkpoint.pth.tar"))

        # do something (save model, change lr, etc.)
        if max_score < valid_logs["weighted_iou"]:
            patience_step = 0
            max_score = valid_logs["weighted_iou"]
            best_epoch = epoch + 1
        ...
        if patience_step >= patience:
            print(
                "\n\nTraining done!  No improvement in {} epochs. Saving final model".format(
                    patience
                )
            )
            if save_name is not None:
